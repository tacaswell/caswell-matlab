function [bins edges] =gofr_2D(max_range,bin_num,range,dim)
...
poses = slice_data(range,dim,poses);
pos_sz = size(poses,1);
...
% $$$             poses(k,plane_dim(2)) > maxes(2)+max_range
        ...
        tmp = repmat(poses(k,plane_dim),pos_sz-1,1);
        dist_tmp = sqrt(sum((poses([1:(k-1) (k+1):end],plane_dim)-tmp).^2,2));
...
data = data(data(:,dim) < range(2) & data(:,dim) > range(1),:);
