function circ_handles = nd2_gui_avg(filename)
...
ac = changer('fr avg',[960 30] ,fh,@adjust_fr_avg);
ps = save_pram('save param',[1040 30],fh,@psave);
...
function int = adjust_fr_avg(int)
    if int<0
        int = 0;
    end
...
function fill_buffer(j)
    ...
    for k = 1:(fr_avg-1)
        img = img +  extract_image(r,buffer_index(j)+k);
    end
