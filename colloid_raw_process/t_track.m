function tracks = t_track(xyzs,maxdisp,param)
%o function tracks = t_track(xyzs,maxdisp,param)
%o summary: my attempts to optomize the matlab tracking/learn the code
%see file for the rest of the header
...
if notnsqrd
    ...
    %only looking at the max and min values of the particles on the edges
    %didn't make sense
...
%    eyes = eyes + res(i);
    ...
    if m > 0
        ...
        if notnsqrd
            ...
            %            tdimm = (tmax-tmin+1)';
            %            for tk = 1:dim
            %                tabi(:,tk) = abi(:,tk) - tmin(tk);
            %                tabpos(:,tk) = abpos(:,tk) - tmin(tk);
            %                tsi = tsi + tabi(:,tk).*tcoff;
            %                tspos = tspos + tabpos(:,tk).*tcoff;
            %                tcoff = tdimm(tk).*tcoff;
            %            end
            %
            %
            %
            ...
            for j=1:dim
             ...
             %           tdimm-dimm
             %           sum(sum(tabi-abi))
             %           sum(sum(tabpos-abpos))
             %           sum(sum(si-tsi))
             %           sum(sum(spos-tspos))
             %           return

             %some how doing it with the for loop is faster for n=2
             %*sulks*
...
%                     cub = cub(find(cub(:,deg(j)) < dimm(deg(j))),:);
%                 end
                 for j=deg
                    cub = cub(find(cub(:,j)<dimm(j)),:);
                    fprintf(2,'debug this: cub = cub(find(cub(:,j)<dimm(j)),:); ')
                 end
             end
             %%[i do not understand what this section does]
            ...
            %            tstrt = zeros(nblocks,1) -1;
            % tfnsh = zeros(nblocks,1);
            ...
            %p_indx = [0; find(diff(ed)~=0); size(si,1)];
            %
            %tstrt(ed(p_indx(1:end-1)+1)) =p_indx(1:end-1)+1;
            %tfnsh(ed(p_indx(1:end-1)+1)) =p_indx(2:end);
            %once again, my way is slower
            ...
            for j=1:n
                ...
                if ngood ~= 0
                    ...
                    %                    map = map(2:end);
                    %don't needs this if using the empty matrix like I
                    %changed above
...
%                     end
            ...
            %xmat = reshape(0:(ntrack*m-1),ntrack,m)';
            %at some threshold this is faster
            ...
            %we explictly set the size of xmat in the preallocation
            %above, no reas to get it again
%            whos ymat
%            whos xmat
%            disp(m)
            ...
            for d=1:dim
                ...
                % else
                    ...
                    %end
                %just prset dq to 0 and it works fine due to promisquity
                %of matlab and removes flow control
            end
            ...
            %i don't think this is really nessecary
        ...
        if nontrivial
            ...
            %no longer nessecary due to above initialization of bonds/bondlen
   ...
   %                 size(w)
                    ...
                    donea = 0;
                    doneb = 0;
                    %%change true to ttrue because true is a built in and
                    ...
                    %what is the point of this conditional? it will
                    %always evaluate to true because we explicitly set
                    %donea~=adda
                    while ttrue==0
...
%                en
                ...
                while checkflag ~= 2
                    ...
                    while who ~= -1
                        who                        
                        if pt(who+1) ~= fi(who+1) %if there is more than
                                                  %one bond
                            ...
                            % check this -1 
                            %extarct them with some weight of if they
                            %have been looked at?
                            ngood = length(w);
                            if ngood > 0
...
if goodenough > 0
    ...
    if nkeep == 0
        ...
        disp('the code broke at line 1046')
        return
    end
    if nkeep < n
        bigresx = bigresx(:,wkeep);
