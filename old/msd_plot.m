function [disp nm_pnts fit] = msd_plot
%%set up junk
    part_sz = 13;
    threshold = 100;
    max_disp = part_sz;
    ...
    %            'room_tmp.nd2'];
     filename = ['/home/tcaswell/collids/data/polyNIPAM_batch_1/20080522/' ...
                'hot_tmp_jammed_2.nd2'];
    ...
    tracks = get_tracks;
    [disp nm_pnts] = get_msd(tracks,15);
    disp = disp*((6.45/mag)^2); %40xlens
    figure
    plot(disp,'x');
    figure
    plot(nm_pnts,'x');
    ...
    if make_fig_t
        ...
        errorbar(mtime*(1:num_points),disp,sqrt(disp./nm_pnts),'xr');
        
        
        pnts = mtime*(1:num_points);
        plot(pnts,fit(1) + (pnts)*fit(2),'r--');
